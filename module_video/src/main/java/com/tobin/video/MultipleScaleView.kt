...
class MultipleScaleView: View {
    ...
    companion object {
        ...
        private const val TAG = "MultipleScaleView"
    }
    ...
    constructor(context: Context, attributeSet: AttributeSet?, defStyleAttr: Int): super(context, attributeSet, defStyleAttr) {
        ...
        Timber.tag(TAG).v("xml最小值 $unitVal")
        mUnitValue = if (unitVal < 1e-6) {
            1f
        } else {
            unitVal
        }
        Timber.tag(TAG).v( "最小值 $mUnitValue")
                        ...
                        if (isInVisibleArea(x)) {
                            if (mOnlyTenScaleLineAble) {
                                ...
                                if (index % 10 == 0) {
                                    ...
                                    canvas.drawText(calculateScaleValue(index) , x.toFloat(), baseline, mTextPaint)
                                }
                            } else {
     ...
     * @param index 第n个刻度线
     */
    private fun calculateScaleValue(index: Int): String {
    ...
    override fun computeScroll() {
        if (mScroller.computeScrollOffset()) {
            ...
            if (isFilingFinsh) {
                ...
                when {
                    mScroller.finalX < mLeftBorder - mShowWidth / 2 -> {
                        Timber.tag(TAG).v("超过最左边")
                        doScroll(mLeftBorder - mShowWidth / 2 - mScroller.finalX, 500)
                    }
                    mScroller.finalX > mLeftBorder + mAllWidth - mShowWidth / 2 -> {
                        Timber.tag(TAG).v("超过最右边")
                        doScroll(mLeftBorder + mAllWidth - mShowWidth / 2 - mScroller.finalX, 500)
                    }
                    else -> {
                        // fling滑动结束值回调
